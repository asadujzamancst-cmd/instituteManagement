...
export default function TeacherPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res: Response;
      if (editingTeacher) {
        res = await fetch(`http://127.0.0.1:8000/teacher/teacher/${editingTeacher.teacher_id}/`, {
          method: "PATCH",
          body: formData,
        });
      } else {
  ...
  const handleDelete = async (id: number) => {
    ...
    setError("");
    try {
      const res = await fetch(`http://127.0.0.1:8000/teacher/teacher/${id}/`, {
        method: "DELETE",
      });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {teachers.map(t => (
          <div key={t.teacher_id} className="border rounded p-4 bg-gray-100 dark:bg-gray-800 shadow">
            ...
            {t.image && <img src={`http://127.0.0.1:8000${t.image}`} alt={t.teacher_name} className="w-full h-40 object-cover mb-2 rounded border hover:opacity-80 transition" />}
